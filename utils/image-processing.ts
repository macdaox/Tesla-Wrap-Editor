...
/**
 * Creates a mask image where the background (connected to 0,0) is transparent,
 * and the isolated regions (car body) are opaque.
 * Used for "Full Wrap" clipping.
 */
export function createBackgroundMask(
  ...
  // We want: Background -> Transparent (Alpha 0)
  //          Car Body -> Opaque (Alpha 255)
  // Initialize with Opaque White (or whatever, only Alpha matters for clipPath usually, but let's be safe)
  // Actually, we can just modify the original data if we want, or create new.
  // Let's create new to be clean.
  const maskImgData = new ImageData(width, height);
  const maskData = maskImgData.data;
  
  // Fill with Opaque Black (Visible)
  for (let i = 0; i < maskData.length; i += 4) {
      maskData[i] = 0;     // R
      maskData[i + 1] = 0; // G
      maskData[i + 2] = 0; // B
      maskData[i + 3] = 255; // Alpha (Visible)
  }
  ...
  // Put data into a temp canvas to get DataURL
